  ...
  const strategies = {
    ...
    data: async () => {
      if (!args.selector?.length) {
        console.error("Error: --selector is a required option for --data.");
        Deno.exit(1);
      }
    ...
    attribute: async () => {
      if (!args.selector?.length) {
        console.error("Error: --selector is a required option for --attribute.");
        Deno.exit(1);
      }
      const selector = args.selector.startsWith(XPATH_PREFIX)
        ? xPathToCss(args.selector)
    ...
    existance: async () => {
      if (!args.selector?.length) {
        console.error("Error: --selector is a required option for --existance.");
        Deno.exit(1);
      }
      const selector = args.selector.startsWith(XPATH_PREFIX)
        ? xPathToCss(args.selector)
    ...
    text: async () => {
      if (!args.selector?.length) {
        console.error("Error: --selector is a required option for --text.");
        Deno.exit(1);
      }
      const selector = args.selector.startsWith(XPATH_PREFIX)
        ? xPathToCss(args.selector)
