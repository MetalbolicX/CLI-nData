...
def plot_scatter(coordinates: LineScatterCoordinates, title: str, xlabel: str, ylabel: str) -> None:
    x = [coordinates['x'] for coordinates in coordinates]
    y = [coordinates['y'] for coordinates in coordinates]
    fig = tpl.figure()
...
def plot_time_series(data: LineScatterCoordinates, title: str, xlabel: str, ylabel: str) -> None:
    # Example implementation for time series (can be customized further)
...
def main() -> None:
    ...
    if args.type in plot_functions:
        ...
        print(f"Plot type '{args.type}' is not supported.")
